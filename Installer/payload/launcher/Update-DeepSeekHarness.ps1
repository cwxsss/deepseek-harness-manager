...
function Save-ProfileSnapshot {
    ...
    New-Item -ItemType Directory -LiteralPath $script:snapshotRoot -Force | Out-Null
    foreach ($source in @(
        ...
        if (Test-Path -LiteralPath $source) {
            ...
            New-Item -ItemType Directory -LiteralPath ([System.IO.Path]::GetDirectoryName($destination)) -Force | Out-Null
...
$latestCore = (& $corepack pnpm --color never view '@deepseek-ai/dsh' version).Trim()
if ($LASTEXITCODE -ne 0 -or $latestCore -eq '') { throw '无法读取 DeepSeek Harness 的最新版本。' }
$latestWebUi = (& $corepack pnpm --color never view '@linxin666/dsh-web-ui-all' version).Trim()
if ($LASTEXITCODE -ne 0 -or $latestWebUi -eq '') { throw '无法读取 Web UI 的最新版本。' }
